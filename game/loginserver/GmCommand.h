...
#define RETURN_FALSE_IF_TARGET_IS_SERVER		\
	if ( m_eTargetType == E_GM_COMMAND_TARGET_TYPE_SERVER )		\
	{															\
		SetErrorMsg( format_error( "Target Type [Server] Not Support") );	\
		return false;											\
	}
...
public:
	...
	:m_dwAutoId(0), m_strOpr(""), m_eTargetType(E_GM_COMMAND_TARGET_TYPE_SERVER),
	 m_dwTargetId(0), m_dwStartTime(0), m_dwEndTime(0), m_bDirty(false),
	 m_strBackup(""), m_eStatus(E_GM_COMMAND_STATUS_NEW)
	{
	...
	virtual bool Cancel()
	{
		...
		if ( m_eStatus == E_GM_COMMAND_STATUS_RUNNING )
		{
			...
			this->SetStatus(E_GM_COMMAND_STATUS_ERROR);
			return false;
		}
		return false;
	}
	...
	virtual bool AfrHandler() { return true; }
	virtual bool PreHandler() { return true; } 
