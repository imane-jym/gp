...
bool CenterDBCtrl::ExistsIndex( const char* table, const char* index )
{
	QueryResult* result = m_db->PQuery(
		...
		table, index, g_strOperateDatabaseName.c_str()
	);
...
bool CenterDBCtrl::ExistsColumn( const char* table, const char* column )
{
	QueryResult* result = m_db->PQuery(
		...
		table, column, g_strOperateDatabaseName.c_str() );
	if ( result )
	{
		delete result;
		return true;
...
bool CenterDBCtrl::ExistsTable( const char* table )
{
	QueryResult* result = m_db->PQuery(
		...
		g_strOperateDatabaseName.c_str(), table
	);
	if ( result )
	{
		delete result;
...
bool CenterDBCtrl::InitCenterDB( DatabaseMysql* db, uint8 byLoginServerId )
{
	...
	if(!ExistsColumn("player_info","card_level")){
		m_db->PExecute("ALTER TABLE player_info ADD COLUMN card_level varchar(64) not null default '0,0,0,0,0,0,0' ");
	}
	if(!ExistsColumn("player_info","ability")){
		m_db->PExecute("ALTER TABLE player_info ADD COLUMN ability int unsigned not null default 0 ");
	}

	if(!ExistsColumn("player_info","limit_role")){
		m_db->PExecute("ALTER TABLE player_info ADD COLUMN limit_role tinyint not null default 0 ");
	}
	...
	if ( !ExistsColumn( "goods_info", "platform_type" ) )
		m_db->PExecute( "ALTER TABLE goods_info ADD COLUMN platform_type INT UNSIGNED NOT NULL" );
...
//		m_db->PExecute( "ALTER TABLE gm_cmd ADD COLUMN params VARCHAR(512)	CHARACTER SET utf8 NOT NULL" );
...
//
//	result = m_db->PQuery(
//		"select MAX(role_id) from re_passport_player"
//	);
//
//	if ( !result )
//	{
...
bool CenterDBCtrl::BackupPassportOfRole( DatabaseMysql* dstDB, uint32 dwRoleId )
{
	...
	if ( !result )
	{
		IME_SQLERROR( "Error Backup Passport To GameServer, passportId=%llu", dwPassportId );
		return false;
	}
	field = result->Fetch();
...
//		{
			return m_db->PExecute( "update player_info set name='%s', status = %u,  gm_auth = %u ,progress=%u, level=%u, "
					...
					"quest=%u, card_level='%s', ability=%u where role_id=%u",
					strRoleName.c_str(), byStatus, byGm, dwProgress, dwLevel, dwGold, dwDiamond, dwCurStage, dwCurTrain, dwVipLevel, dwVipExp,
					dwStamina, dwEnergy, dwMainQuestId, cardLevel.c_str(), dwAbility, dwRoleId
			);
//		}
//		else
//		{
...
//		}
	...
	else
	{
		...
		return m_db->PExecute( "insert into player_info( role_id, name, gm_auth, status, progress, level, "
				"gold, diamond, cur_stage, cur_train, vip_level, vip_exp, stamina, energy, quest, card_level, ability ) values( %u, '%s', %u, %u, %u, %u, %u, %u, %u, %u, %u, %u, %u, %u, '%s', %u,%u )",
				dwRoleId, strRoleName.c_str(), byGm, byStatus, dwProgress, dwLevel, dwGold, dwDiamond, dwCurStage, dwCurTrain, dwVipLevel, dwVipExp,
				dwStamina, dwEnergy, cardLevel.c_str(), dwAbility, dwMainQuestId
		);
	}
}
...
bool CenterDBCtrl::GetExtendReward( const char* strTableName, uint8 byIdType, uint32 dwRoleId, uint32& dwItemId, uint32& dwItemCnt, uint8& byState )
{
	...
	if ( byIdType == 1 )
	{
		// role_id
		result = m_db->PQuery(
			"select item_id, item_cnt, state from %s where id=%u",
			strTableName, dwRoleId );
	}
	else if ( byIdType == 2 )
	{
		// passport_id
		...
		if ( result )
		{
			...
			result = m_db->PQuery(
				"select item_id, item_cnt, state from %s where id=%llu",
				strTableName, ret );
		}
		else
		{
			IME_SQLERROR( "Passport Not Found, roleId=%u", dwRoleId );
			return false;
		}
	}
	else
	{
		IME_ERROR( "Invalid Id Type, type=%u", byIdType );
		return false;
	}
...
bool CenterDBCtrl::SetExtendRewardGot( const char* strTableName, uint8 byIdType, uint32 dwRoleId, uint8 byState )
{
	if ( !ExistsTable( strTableName ) )
	{
		IME_SQLERROR( "Table Not Found When GetExtendReward, name=%s", strTableName );
	...
	if ( byIdType == 1 )
	{
		// role_id
		return m_db->PExecute(
			"update %s set state=%u where id=%u", strTableName, byState, dwRoleId );
	}
	else if ( byIdType == 2 )
	{
		// passport_id
		...
		if ( result )
		{
			...
			return m_db->PExecute(
				"update %s set state=%u where id=%llu", strTableName, byState, ret );
		}
		else
		{
			IME_SQLERROR( "Passport Not Found, roleId=%u", dwRoleId );
...
{
	QueryResult* result = m_db->PQuery(
		"select auto_id, role_id, goods_id, goods_quantity, addition2 from charge_info where state=%u",
...
bool CenterDBCtrl::InsertCharge(
			uint32		dwGoodsId,
			uint32 		dwRoleId,
			uint32		dwNumber
		)
{
	return m_db->PExecute(
		"insert into charge_info(role_id, goods_id, goods_quantity, state) values(%u, %u, %u, 1)", dwRoleId, dwGoodsId, dwNumber
	);
}
...
void CenterDBCtrl::GetGoodsInfoOfGameServer( std::map< uint32, STC_GOODS_INFO>& vGoods, uint32 dwGameServerId )
{
	QueryResult* result = m_db->PQuery(
		...
		"status, limit_day, sort_idx, icon_id, goods_name, description, limit_once, platform_type from goods_info where server_id=%u", dwGameServerId
	);
...
void CenterDBCtrl::UpdateGoodsInfoOfGameServer( std::map<uint32, STC_GOODS_INFO>& vGoods, uint32 dwGameServerId )
{
			...
			it != vGoods.end(); ++it )
	{
		m_db->PExecute( "insert into goods_info(server_id, goods_id, shop_type, buy_type_id, "
				...
				"status, limit_day, sort_idx, icon_id, goods_name, description, limit_once, platform_type) values(%u, %u, "
				"%u, %u, %u, %u, %u, %u, %u, %u, %u, %u, %u, %u, '%s', '%s', %d, %u)",
				dwGameServerId,
				it->second.dwGoodsId,
