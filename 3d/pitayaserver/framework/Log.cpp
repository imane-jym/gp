...
void Log::outTimestampBySeq(FILE *file, const char *str)
{
    time_t t = time(NULL);
    tm* aTm = localtime(&t);
    //       YYYY   year
    ...
    fprintf(file, "%-4d-%02d-%02d %02d:%02d:%02d%s", aTm->tm_year+1900, aTm->tm_mon+1, aTm->tm_mday, aTm->tm_hour, aTm->tm_min, aTm->tm_sec, str);
}
...
void Log::outCharBySeq(const char * seq, const char * str, ...)
{
    if (!str)
        return;
    ...
    if (charLogfile)
    {
        outTimestampBySeq(charLogfile, seq);
        va_list ap;
        va_start(ap, str);
        vfprintf(charLogfile, str, ap);
        fprintf(charLogfile, "\n" );
